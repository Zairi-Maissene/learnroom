...
export class ClassroomDetailComponent {
  ...
  students$: Observable<{ teacher: Teacher; students: Student[] } | null> =
    new Observable();
  ...
  ngOnInit() {
    ...
    this.classroom$.subscribe((classroom) => {
      this.assignments$ = this.classroomService.getAssignments(
        classroom?.id || '',
      );
    ...
    const combinedSearch$ = combineLatest([
      this.searchForm
        .get('searchTerm')
        ?.valueChanges.pipe(debounceTime(200), startWith('')) || of(''),
      this.classroom$.pipe(
