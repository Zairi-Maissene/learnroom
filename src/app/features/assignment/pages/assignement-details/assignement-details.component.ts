...
export class AssignementDetailsComponent implements OnInit {
  ...
  responseAssignment$: Observable<ResponseAssignement> =
    new Observable<ResponseAssignement>();
  ...
  constructor(
    ...
    this.submitAssignmentForm.controls['description'].setErrors({
  ...
  ngOnInit(): void {
    this.route.params
      ...
      .subscribe();
    if (!this.isTeacher) {
      this.responseAssignment$ = this.assignmentService.getResponseAssignment(
        this.assignmentId,
      );
  ...
  onScoreChange(event: Event, index: number) {
    ...
    const updatedResponse: ResponseAssignement[] = this.responsesAssignment.map(
      (item, i) => {
        if (i === index) {
  ...
  onSubmit: any = () => {
    this.assignmentService.updateResponseAssignment(
      this.assignmentId,
